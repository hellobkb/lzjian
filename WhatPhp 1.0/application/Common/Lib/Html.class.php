...
class Html extends AppframeController {
   ...
   /*  protected function _initialize() {
    ...
    public function show($data = '', $array_merge = 1) {
        ...
        C("TMPL_PARSE_STRING.__TMPL__",__ROOT__."/".$tmpl_path.$theme."/");
        ...
        $content_output = new \Content\Fields\content_output($this->modelid);
        //获取字段类型处理以后的数据
        ...
        }$seo_keywords=$output_data['seokeywords'];
        $seo = seo($catid, $output_data['seotitle']?$output_data['seotitle']:$output_data['title'], $output_data['seodescription'], $seo_keywords);
        //内容页模板
        ...
        if ($data['paginationtype'] > 0) {
            ...
            if ($content_page_pos !== false) {
                ...
                foreach ($pageurls as $page => $urls) {
                   ...
                   //先不生成静态
                    $this->buildHtml(str_replace(".html","",$urls['path']), SITE_PATH, $templatePath);
                }
                return true;
            }
        }
        ...
        do {
            ...
            //生成静态页现在先不做
            $this->buildHtml(str_replace(".html","",$category_url["path"]), SITE_PATH, $templatePath);
    ...
    public function category($catid) {
        ...
        do {
            ...
            if ($repagenum && !$GLOBALS['dynamicRules']) {
                ...
                $GLOBALS['dynamicRules'] = $sitearr['site_host'] . "index.php?a=lists&catid={$catid}&page=*";
            }
            ...
            $this->assign(C("VAR_PAGE"), $page);
            $this->buildHtml(str_replace(C('HTML_FILE_SUFFIX'),'',$category_url["path"]), SITE_PATH, $template);
    ...
    public function index() {
        ...
        $this->assignInitialize();
        //模板处理
        $tp = explode('.', "index.php", 2);
       
        //找到了首页的DIR地址
        $template = parseIndexTemplateFile("Index/{$tp[0]}");
    ...
    protected function generateCategoryUrl($catid, $page = 1) {
    	$url = new \Common\Lib\Url();
        return $url->category_url($catid, $page);
    }
