...
class Url {
    ...
    public function index($page = 1) {
        ...
        $page = max($page, 1);
        //URL规则ID 
        $index_ruleid =11;  //$config['index_urlruleid'];           //没有这个
        ...
        $url = array(
            "url" => $sitearr['site_host'] . ($page > 1 ? $urlrule[1] : $urlrule[0]),
            "path" => "",
        );

        $url["url"] = str_replace('{page}', $page, $url["url"]);
        $parse_url = parse_url($url['url']);
        $url['path'] = str_replace(__ROOT__,'',$parse_url['path']);

        //用于分页使用
        $url['page'] = array(
            "index" => $sitearr['site_host'] . $urlrule[0],
            "list" => $sitearr['site_host'] . $urlrule[1],
        );
        ...
        if (in_array(basename($url["url"]), array('index.html', 'index.htm', 'index.shtml', 'index.php'))) {
        	...
        	$url['path'] = str_replace(array(".html",'.php'),"",$url['path']);
        }
    ...
    public function show($data = '', $page = 1) {
       ...
       /*  $config = cache("Config");   //没有 */
        ...
        if ($setting['content_ishtml']) {
            ...
            $htmlrule=C('CX_RULE_URL');
             
            $movehtml = M('urlrule')->where(" urlruleid =".$htmlrule[$setting['show_ruleid']])->find();
             
            $moveurlrule =  $movehtml['urlrule'];
            $moveurlarr = explode("|", $moveurlrule);
            
            $moveurl = array(
            		"url" => ($page > 1 ? $moveurlarr[1] : $moveurlarr[0]),
            );

            $moveurl["url"] = str_replace('{page}', $page, $moveurl["url"]);
            ...
            if (!isset($parse_url['host'])) {
            	...
            	$replace_r[] = $catid;
            	//替换变量
            	$moveurl['url'] = $sitearr['site_host'] . str_replace($replace_l,$replace_r,$moveurl['url']);
            }
            
           
            //将URLrule 拆分，加入title, 然后拼接

            $urlrulearr = explode("|", $urlrule);
            $temp = explode("/", $urlrulearr[0]);
            
            //var_dump($urlrulearr);exit;
            
            $urlrule = $temp[0]."/".($data['seourl'] ? $data['seourl'].".html": "".$temp[1])."|".$urlrulearr[1];

            
           /*  if($data['ProductData']['seourl']){ //添加
            	$urlrule = $temp[0]."/".$data['ProductData']['seourl']."-".$temp[1]."|".$urlrulearr[1];
            }else if(!$data['ProductData']['seourl'] && $data['seourl']){    //修改
            	$urlrule = $temp[0]."/".$data['seourl']."-".$temp[1]."|".$urlrulearr[1];
            }else{
            	$urlrule = $temp[0]."/".$temp[1]."|".$urlrulearr[1];
            } */
        	...
        	//判断是否有加域名
        	if (!isset($parse_url['host'])) {
        		$moveurl['url'] = $sitearr['site_host'] . $moveurl['url'];
        	}
        ...
        //判断是否有加域名
        if (!isset($parse_url['host'])) {
            $url['url'] = $sitearr['site_host'] . $url['url'];
            $url['page']['index'] = $sitearr['site_host'] . $url['page']['index'];
            $url['page']['list'] = $sitearr['site_host'] . $url['page']['list'];
        }
    ...
    public function category_url($catid, $page = 1, $category_ruleid = false) {
        ...
        if (!isset($_category_url[$catid])) {
            ...
            if($_SESSION['editModel'] ==1){
                $htmlrule=C('CX_RULE_URL');
                if(isset($htmlrule[$category_ruleid])){
                    $category_ruleid = $htmlrule[$category_ruleid];
                }
            }
            //取得规则
        ...
        if ($setting['ishtml']) {  //生成静态
            ...
            //用户选择了静态页面，也要找到动态的链接保存
            //根据静态的category_ruleid 得到动态的URL
 
            $htmlrule=C('CX_RULE_URL');
            $map1['urlruleid']=$htmlrule[$setting['category_ruleid']];
            $movehtml = M('urlrule')->where($map1)->find();
            ...
            $urlrule = explode("|",$urlrule);
            /*
            foreach(S('languages') as $defkey=>$defvalue){
            	if($defvalue['def'] ==1){
            		$def_lan = $defvalue;
            		break;
            	}
            ...
            if($def_lan['lsign'] !=LANG_SET){
            	$lan_arr = explode("-",LANG_SET);
            	$urlrule[0] = $lan_arr[1]."/".$urlrule[0];
            	$urlrule[1] = $lan_arr[1]."/".$urlrule[1];
            }
            */
            $urlrule = implode("|", $urlrule);
            
        }else{  //选择的是动态的，也要保存动态的链接
            ...
            if(stristr($moveurl['url'],'leuu') !== false){
                eval("\$moveurl['url']=$moveurl[url];");
                if(stristr($moveurl['url'],'index.php') !== false)
                    $moveurl['url'] = stristr($moveurl['url'],'index.php');
            }
        ...
        if(stristr($urlrule[0],'leuu') !== false){
            eval("\$urlrule[0]=$urlrule[0];\$urlrule[1]=$urlrule[1];");
            if(stristr($urlrule[0],'index.php') !== false)
                $urlrule[0] = stristr($urlrule[0],'index.php');
            if(stristr($urlrule[1],'index.php') !== false)
                $urlrule[1] = stristr($urlrule[1],'index.php');
        }
        ...
        $url["url"] = str_replace('//', '/', $url["url"]);
        $url['page']['index'] = str_replace('//', '/', $url['page']['index']);
        $url['page']['list'] = str_replace("//", "/", $url['page']['list']);
    ...
    public function tags($data = '', $page = 1, $ruleid = 0) {
        ...
        if (!isset($_tags[$guid])) {
            ...
            //url规则
            $urlrule = $this->urlrules[$ruleid ?  : $config['tagurl']]['urlrule'];    //没有这个选项
    ...
    // 静态地址
    private function jturl(){
        // 生成静态地址
        return '{$categorydir}{$catdir}/index.html|{$categorydir}{$catdir}/index_{$page}.html';
    }
