	...
	function delete(){
		if(isset($_GET['id'])){
			...
			if ($this->comments_model->where("id=$id")->delete()!==false) {
				adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_DEL').L('ADMINLOG_COMMENTS'));
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
	...
	function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			if ( $this->comments_model->where("id in ($ids)")->save($data)!==false) {
				adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_AUDIT').L('ADMINLOG_COMMENTS'));
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["uncheck"]){
			...
			$ids=join(",",$_POST['ids']);
			if ( $this->comments_model->where("id in ($ids)")->save($data)!==false) {
				adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_NOAUDIT').L('ADMINLOG_COMMENTS'));
				$this->success("取消审核成功！");
			} else {
				$this->error("取消审核失败！");
			}
