...
class NavcatController extends AdminbaseController {
		...
		if (IS_POST) {
			...
			if ($this->navcat_model->create()) {
				if ($this->navcat_model->add()) {
					adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_ADD').L('ADMINLOG_CATEGORY').$_POST['name']);
		...
		if (IS_POST) {
			...
			if ($this->navcat_model->create()) {
				if ($this->navcat_model->save() !== false) {
					adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_SAVE').L('ADMINLOG_CATEGORY').$_POST['name']);
	...
	function delete(){
		$id = intval(I("get.id"));
		$info = $this->navcat_model->where("navcid=$id")->find();
		if ($this->navcat_model->where("navcid=$id")->delete()!==false) {
			$nav_obj=D("Common/Nav");
			$nav_obj->where("cid=$id")->delete();
			adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_DEL').L('ADMINLOG_CATEGORY'));
			$this->success("删除成功！");
		} else {
			$this->error("删除失败！");
		}
