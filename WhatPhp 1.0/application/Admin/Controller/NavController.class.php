...
class NavController extends AdminbaseController {
		...
		if (IS_POST) {
			...
			if ($this->nav_model->create($data)) {
				...
				if ($result!==false) {
					...
					adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_ADD').L('ADMINLOG_MENU'));
		...
		if (IS_POST) {
			...
			if ($this->nav_model->create($data)) {
				if ($this->nav_model->save() !== false) {
					adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_SAVE').L('ADMINLOG_MENU'));
	...
	public function listorders() {
		...
		if ($status) {
			adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_ORDER').L('ADMINLOG_MENU'));
			$this->success("排序更新成功！");
		} else {
			$this->error("排序更新失败！");
		}
	...
	public function delete() {
		...
		if ($this->nav_model->delete($id)!==false) {
			adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_DEL').L('ADMINLOG_MENU'));
			$this->success("删除菜单成功！");
		} else {
			$this->error("删除失败！");
		}
