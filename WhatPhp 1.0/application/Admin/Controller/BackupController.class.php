...
class BackupController extends AdminbaseController
{
    ...
    public function index_post(){
    	if (IS_POST || isset($_GET['dosubmit']))
    	{
            ...
            adminlog($_SESSION['ADMIN_ID'],L('ADMIN_BACUPS').$this->backup_name);
    		exit();
    	}
    }
    ...
    public function import()
    {
        ...
        { 
            adminlog($_SESSION['ADMIN_ID'],L('ADMIN_DR_BACUPS').$this->backup_name);
    ...
    public function del_backup()
    {
        ...
        import('@.ORG.Dir');
        if(strpos($_GET['backup'],'.') !== false || strpos($_GET['backup'],'/') !== false || strpos($_GET['backup'],'\\') !== false)
            $this->error('非法参数');
        $file = SITE_PATH . $this->backup_path . $_GET['backup'].'/';
        $dir=new \Dir($file);
        $dir->delDir($file);
        adminlog($_SESSION['ADMIN_ID'],L('DELETE').$_GET['backup']);
        $this->success('操作成功！');
    }
    ...
    public function download()
    {
        ...
        $file = isset($_GET['file']) && trim($_GET['file']) ? trim($_GET['file']) : $this->
            error('请选择备份文件！');
        $sqlfile = pathinfo($file);
        if($sqlfile['extension'] != 'sql')
            $this->error('请选择正确的文件名称！');
        ...
        if (file_exists($sql_file))
        {
            ...
            readfile($sql_file);
            adminlog($_SESSION['ADMIN_ID'],L('DOWNLOAD').$backup_name);
        } else
        {
            $this->error('文件不存在！');
        }
