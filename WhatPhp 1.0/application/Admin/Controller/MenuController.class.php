...
class MenuController extends AdminbaseController {
    ...
    public function index() {
        ...
        $categorys = $tree->get_tree(0, $str);//print_r($categorys);exit;
    	...
    	if (IS_POST) {
    		if ($this->menu_model->create()) {
    			if ($this->menu_model->add()!==false) {
                    ...
                    adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_ADD').L('ADMINLOG_HCATE').$menu_name);
    ...
    public function delete() {
        $id = intval(I("get.id"));
        $menu = $this->menu_model->where(array("parentid" => $id))->find();
        $count = $this->menu_model->where(array("parentid" => $id))->count();
        if ($count > 0) {
            $this->error("该菜单下还有子菜单，无法删除！");
        }
        if ($this->menu_model->delete($id)!==false) {
            adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_DEL').L('ADMINLOG_HCATE').$menu['name']);
            $this->success("删除菜单成功！");
        } else {
            $this->error("删除失败！");
        }
    	...
    	if (IS_POST) {
    		if ($this->menu_model->create()) {
    			if ($this->menu_model->save() !== false) {
    				...
    				adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_SAVE').L('ADMINLOG_HCATE').$menu_name);
    				$this->success("更新成功！");
    			} else {
    				$this->error("更新失败！");
    			}
    ...
    public function listorders() {
        ...
        if ($status) {
            adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_ORDER').L('ADMINLOG_HCATE'));
            $this->success("排序更新成功！");
        } else {
            $this->error("排序更新失败！");
        }
    ...
    public function spmy_export_menu(){
    	...
    	foreach ($menus as $m){
            ...
            adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_COPY').L('ADMINLOG_HCATE'));
    	...
    	foreach ($apps as $app){
    		if(is_dir(SPAPP.$app)){
    			...
    			foreach ($lang_dirs as $lang_dir){
                    ...
                    adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_CATE_MORE_LB'));
    			}
