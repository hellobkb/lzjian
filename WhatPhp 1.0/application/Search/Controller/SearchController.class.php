    ...
    public function create() {
        ...
        if (isset($_GET['start'])) {
            ...
            if ($modelid) {
                ...
                //取得总数
                if (!isset($_GET['total'])) {
                    $count = $ContentDb->where(array("status" => 99,"is_del"=>0, "isdelay"=>0))->count();
                    //信息总数
                ...
                $data = $ContentDb->relation(true)->where(array("status" => 99 ,"is_del"=>0, "isdelay"=>0))->order(array("id" => "ASC"))->limit($offset . "," . $pagesize)->select();
