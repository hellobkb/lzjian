...
class SearchModel extends Model {
    ...
    private function dataHandle($data, $text = '') {
        ...
        $data = \Common\Lib\Input::forSearch($data);
        $data = \Common\Lib\Input::deleteHtmlTags($data);
        //搜索配置
    ...
    public function search_api($id = 0, $data = array(), $modelid, $action = 'add') {
        ...
        if ($action == 'add' || $action == 'updata') {
            ...
            //在此拼接是必填的字段，不光是title和keywords,查找模型中哪些是搜索字段
            $textcontent = M('ModelField')->where(" modelid = $modelid and issearch =1")->select();
             foreach($textcontent as $textsearch){
             	$fulltextcontent .= $data[$textsearch['field']];
             }
