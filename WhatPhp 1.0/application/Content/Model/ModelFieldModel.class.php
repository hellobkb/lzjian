...
class ModelFieldModel extends Model {
    ...
    private $fieldPath = '';
    //不显示的字段类型（字段类型）
    public $not_allow_fields = array('catid', 'typeid', 'title', 'keyword', 'template', 'username', 'tags','seourl');
    //允许添加但必须唯一的字段（字段名）
    public $unique_fields = array('pages', 'readpoint', 'author', 'copyfrom', 'islink');
    //禁止被禁用（隐藏）的字段列表（字段名）
    public $forbid_fields = array('catid', 'title', 'seourl',/* 'updatetime', 'inputtime', 'url', 'listorder', 'status', 'template', 'username', 'allow_comment', 'tags' */);
    //禁止被删除的字段列表（字段名）
    public $forbid_delete = array('catid', /* 'typeid', */ 'title', 'thumb', 'keyword', 'keywords', 'updatetime', 'tags', 'inputtime', 'posid', 'url', 'listorder', 'status', 'template', 'username', 'allow_comment', 'content','seourl','isdelay');
    //可以追加 JS和CSS 的字段（字段名）
    ...
    public function addField($data) {
        ...
        //$fields_info=M("ModelField")->where(array("modelid"=>$modelid))->field("field")->select();
