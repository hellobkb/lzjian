...
class CategoryModel extends Model {
    ...
    protected $_validate = array(
        ...
        array('catdir', 'checkCatdir', '英文目录名称已存在！', 1, 'callback', 1),
    ...
    public function checkCatdir($catdir, $catid = 0, $parentid = 0, $old_catdir = false, $type = false) {
    	...
    	if($catid){
    		$check = $this->where("catdir='$catdir' and catid!='$catid'" )->find();
    		if($check){
    			return false;
    		}
    	}else{
    		$check = $this->where(" catdir='$catdir'")->find();
    		if($check){
    			return false;
    		}
    	}
    ...
    public function addCategory($post) {
        ...
        $letters = $iconvfunc->gbk_to_pinyin($catname);
    ...
    public function editCategory($post) {
        ...
        $checkdir= $this->checkCatdir($data['catdir'], $catid);
        
        if(!$checkdir){
        	$this->error ="栏目修改失败，请修改英文栏目名称！";
        	return false;
        }
    ...
    public function deleteCatid($catid) {
        ...
        if (is_array($catid)) {
            ...
            foreach ($catList as $cat) {
                //是否存在子栏目
                if ( count(explode(",", $cat['arrchildid'])) >1  ) {  //$cat['child'] && $cat['type'] == 0
            ...
            //是否存在子栏目
            if ( count(explode(",", $catInfo['arrchildid'])) >1 ) {  //$catInfo['child'] && $catInfo['type'] == 0
        ...
        if (is_array($catid)) {
            ...
            foreach ($catid as $cid) {
                ...
                //如果下面有单页也不能删除
                if($catinfo['type'] ==1 )  return false;
            ...
            $childnum = count(explode(',', $catinfo['arrchildid']));
            //如果下面有单页也不能删除
            if($catinfo['type'] ==1 && $childnum > 1) {
                return false;
            }
            if($catInfo['modelid'] > 0){
     ...
     * @return boolean
     */
    public function deleteCatidone($catid, $ids=array()) {
        if (!$catid || !$ids) {
            return false;
        }
        $where = array();
        ...
        $catInfo = $this->where($where)->find();
        //是否存在子栏目
        if ( count(explode(",", $catInfo['arrchildid'])) ==1 || in_array($catid, $ids)) {  //$catInfo['child'] && $catInfo['type'] == 0
            $arrcatid[] = $catid;
            $where['catid'] = array("IN", $arrcatid);
        }else{
            return false;
        }
        ...
        if($catInfo['modelid'] > 0){
            $map['catid'] = array('in',explode(",", $catInfo['arrchildid']));
            $tbname = ucwords(getModel($catInfo['modelid'], 'tablename'));
            if (!$tbname && $catinfo['type'] == 0) {
                return false;
            }
            if ($tbname && $catinfo['type'] == 0 && M($tbname)->where($map)->count()) {
                return false;
            }
        }
