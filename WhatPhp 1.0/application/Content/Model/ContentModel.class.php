...
class ContentModel extends RelationModel {
    ...
    static public function getInstance($modelid) {
        ...
        if (is_null($_instance[$modelid]) || !isset($_instance[$modelid])) {
            //内容模型缓存
            $modelCache = M("Model")->where(array("modelid"=>$modelid))->find();
            if (empty($modelCache)) {
                return false;
            }
            $tableName = $modelCache['tablename'];
     ...
     * @return type
     */
    public function getRelationName($tableName = '') {//echo 11;exit;
        if (empty($tableName)) {
            $tableName = $this->name;
    ...
    public function locking($catid, $id, $userid = 0) {
        ...
        if ($info && $info['userid'] != session('ADMIN_ID')) {
