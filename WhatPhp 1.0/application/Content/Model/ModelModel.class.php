...
class ModelModel extends Model {
    ...
    const mainTableSql = 'Data/Sql/thinkcmf_zhubiao.sql'; //模型主表SQL模板文件
    const sideTablesSql = 'Data/Sql/thinkcmf_zhubiao_data.sql'; //模型副表SQL模板文件
    const modelTablesInsert = 'Data/Sql/thinkcmf_insert.sql'; //可用默认模型字段
    const membershipModelSql = 'Data/Sql/thinkcmf_member.sql'; //会员模型
    ...
    public function addModel($data) {
        ...
        if ($data) {
            ...
            if ($modelid) {
                ...
                //模型菜单
                $date_menu['parentid'] = 171;
                $date_menu['app'] = 'Content';
                $date_menu['model'] = 'Contents';
                $date_menu['action'] = $data['tablename'];
                $date_menu['data'] = "modelid=".$modelid;
                $date_menu['type'] = 1;
                $date_menu['status'] = 1;
                $date_menu['name'] = $data['name'];
                $date_menu['remark'] = '';
                $date_menu['listorder'] = '0';
                $menuid = D("Common/Menu")->add($date_menu);
                if($menuid){
                    $app=$date_menu['app'];
                    $model=$date_menu['model'];
                    $action=$date_menu['action'];
                    $name=strtolower("$app/$model/$action");
                    $menu_name=$date_menu['name'];
                    $mwhere=array("name"=>$name);
                    
                    $find_rule=D("Common/AuthRule")->where($mwhere)->find();
                    if(!$find_rule){
                        D("Common/AuthRule")->add(array("name"=>$name,"module"=>$app,"type"=>"admin_url","title"=>$menu_name));//type 1-admin rule;2-user rule
                    }
                }
                $w_model['modelid'] = $modelid;
                $date_menu_info['menuid'] = $menuid;
                M('Model')->where($w_model)->save($date_menu_info);
        ...
        if ($data) {
            ...
            if ($info['tablename'] != $data['tablename'] && !empty($data['tablename'])) {
                ...
                if (false !== $this->where(array("modelid" => $modelid))->save($data)) {
                    ...
                    S("Model", NULL);
                    //
                    $map_menu = $info['menuid'];
                    $date_menu['action'] = $data['tablename'];
                    $date_menu['name'] = $data['name'];
                    $menuid = D("Common/Menu")->where('id='.$map_menu)->save($date_menu);
                    return true;
                } else {
                    $this->error = '模型更新失败！';
                    return false;
    ...
    public function deleteModel($modelid) {
        ...
        $this->where(array("modelid" => $modelid))->delete();
        //删除该模型菜单
        $map_menu['id'] = $modeldata['menuid'];
        M('Menu')->where($map_menu)->delete();
        //更新缓存
        S("Model", NULL);
        //删除所有和这个模型相关的字段
    ...
    public function sql_split($sql) {
        ...
        if (C("DB_PREFIX") != "cmf_") {
            $sql = str_replace("cmf_", C("DB_PREFIX"), $sql);
        }
        $sql = str_replace("\r", "\n", $sql);
        $ret = array();
        $num = 0;
