 ...
 * @return string
 */
function downfiles($field, $value, $fieldinfo,$catid) {
    //错误提示
    ...
    $str1['str'] = '<input name="info[' . $field . ']" type="hidden" value="1">
        ...
        <legend>文件列表</legend>';
    $str1['str'] .= $list_str;
    $str1['str'] .= '<ul id="' . $field . '" class="picList"></ul>
        </fieldset>
        <div class="bk10"></div>
        ';
    ...
    $module = MODULE_NAME;
    //生成上传附件验证
    $authkey = md5("{$setting['upload_number']},{$setting['upload_allowext']},{$setting['isselectimage']}" . md5(C("AUTHCODE") . $_SERVER['HTTP_USER_AGENT']));
    //后台允许权限设置
...
function change_multifile_admin(uploadid, returnid) {
    ...
    var group = \'权限：' . select($group, $id, 'name="\' + returnid + \'_groupid[]"', '游客') . '\';
...
function add_multifile_admin(returnid) {
    ...
    $(\'#\' + returnid).append(str);
        ...
        $str1['str'] .= $str . "<a herf='javascript:void(0);' class=\"btn\"  onclick=\"javascript:flashupload('{$field}_multifile', '附件上传','{$field}',change_multifile_admin,'{$setting['upload_number']},{$setting['upload_allowext']},$uploadfilesize','{$module}','$this->catid','{$authkey}')\"><span class=\"add\"></span>多文件上传</a>    <a  class=\"btn\" herf='javascript:void(0);'  onclick=\"add_multifile_admin('{$field}')\"><span class=\"add\"></span>添加远程地址</a>$js";
    } else {
        $str1['str'] .= $str . "<a herf='javascript:void(0);'  class=\"btn\" onclick=\"javascript:flashupload('{$field}_multifile', '附件上传','{$field}',change_multifile,'{$setting['upload_number']},{$setting['upload_allowext']},$uploadfilesize','{$module}','$catid','{$authkey}')\"><span class=\"add\"></span>多文件上传</a>    <a herf='javascript:void(0);' class=\"btn\" onclick=\"add_multifile('{$field}')\"><span class=\"add\"></span>添加远程地址</a>";
    }
    return $str1;
}