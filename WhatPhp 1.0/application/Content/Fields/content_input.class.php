...
class content_input {
    ...
    public function setModelid($modelid) {
        ...
        $model_info=$this->model->where("modelid=".$this->modelid)->find();
        if (empty($model_info)) {
            return false;
        }
    ...
    public function get($data, $type = 3) {
        ...
        $tablename=M("Model")->where("modelid=".$this->modelid)->getField("tablename");
        $t_name=ucwords($tablename) . 'Data';
        foreach ($this->fields as $fieldInfo) {
            ...
            if ($issystem) {
                ...
                if(is_array($value)){
                    $this->infoData[$t_name][$field] = implode("," ,$value);
                }else{
                     $this->infoData[$t_name][$field] = $value;
                }
        ...
        if (!isset($this->infoData[$t_name])) {
            $this->infoData[$t_name] = array('id' => 0);
        }
