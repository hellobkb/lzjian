...
class Content {
    ...
    public function where($attr) {
        ...
        $count_arrchildid = count(explode(',', $catinfo['arrchildid']));
        if (isset($attr['catid']) && (int) $attr['catid']) {
            if(strchr($catid,',')!==false){
                $catid = $attr['catid'];
                $where['catid'] = array("IN", $catid);
            }else if ($count_arrchildid > 1) {
    ...
    public function lists($data) {
        ...
        $maporder['catid'] = $data['catid'];
        $sortorder = M('Sortorder')->where($maporder)->find();
        if($sortorder['sort'] && empty($data['order'])){
            $data['order'] = array($sortorder['sort'] => $sortorder['order'] ? $sortorder['order'] : 'DESC' );
        }else if(empty($data['order']) && !$sortorder['sort']) {
