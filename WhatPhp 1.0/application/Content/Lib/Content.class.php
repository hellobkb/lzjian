...
// | thinkcmf 内容处理
// +----------------------------------------------------------------------
// | Copyright (c) 2012-2014 http://www.thinkcmf.com,
// +----------------------------------------------------------------------
// | Author: 
// +----------------------------------------------------------------------
...
class Content extends Components {
    ...
    public function add($data = '') {
        ...
        if (!defined('IN_ADMIN') || (defined('IN_ADMIN') && IN_ADMIN == false)) {
            ...
            $data['username'] = M('Users')->where(array("id"=>session('ADMIN_ID')))->getField("user_login");
            $data['sysadd'] = 0;
        } else {
            //添加用户名
            $data['username'] = M('Users')->where(array("id"=>session('ADMIN_ID')))->getField("user_login");
        ...
        $model = \Content\Model\ContentModel::getInstance($this->modelid);
        $content_input = new \Content\Fields\content_input($this->modelid);
        //保存一份旧数据
        $oldata = $data;
        ...
        $olddata['moveurl'] = $urls['moveurl'];
        $data['moveurl'] = $urls['moveurl'];
        //更新url
        $model->token(false)->where(array('id' => $id))->save(array('url' => $data['url'],'moveurl'=>$data['moveurl']));
       
        $content_update = new \Content\Fields\content_update($this->modelid);
        ...
        //生成相关
        $generatelish = 0;
       	$this->Html = new \Common\Lib\Html();    //初始化Html类
    ...
    public function edit($data = '') {
        ...
        include $this->fields . 'content_input.class.php';*/
        $content_input = new \Content\Fields\content_input($this->modelid);
        //保存一份旧数据
        $oldata = $data;
    ...
    public function check($catid = '', $id = '', $status = 99) {
        if (empty($catid) && empty($id)) {
            ...
            //模型ID
            $this->modelid = getCategory($catid, 'modelid');
            $data = \Content\Model\ContentModel::getInstance($this->modelid)->relation(true)->where(array('id' => $id, 'catid' => $catid))->find();
        }$model=\Content\Model\ContentModel::getInstance($this->modelid);
        $model->dataMerger($data);
        C('TOKEN_ON', false);
        //是否生成HTML
    ...
    public function delete($id = '', $catid = '') {
        ...
        $content_update = new \Content\Fields\content_delete($this->modelid);
        $content_update->get($data);
        //删除内容
        $model->relation(true)->where(array('id' => $id))->delete();
        //删除附件
        //$Attachment = service('Attachment');
    ...
    public function deleteHtml($catid = '', $id = '', $inputtime = '', $prefix = '') {
       ...
       \Content\Model\ContentModel::getInstance($this->modelid)->dataMerger($data);
        //获取信息生成地址和url
        $urls = $this->generateUrl($data);
        $fileurl = $urls['path'];
        //删除静态文件
    ...
    public function othor_catid($othor_catid, $linkurl, $data, $modelid) {
        ...
        foreach ($othor_catid as $cid) {
            ...
            if ($modelid == $mid) {//相同
                ...
                $newid = \Content\Model\ContentModel::getInstance($mid)->relation(true)->add($dataarray);
            }
        }
        return true;
    }
