...
class CreatehtmlController extends AdminbaseController{
	...
	public function update_post(){
		...
		if(!$_GET['process']){
			...
			sp_clear_cache();
			$this->success("缓存更新完成,开始更新url...",U("Createhtml/update_post","process=4"));
			...
			if(!$set_catid){
				...
				if(!isset($catid_arr[$autoid])){
					$this->success("更新栏目页完成" ,U("Createhtml/update_post","process=5"));
					exit;
				}
			...
			if (!isset($set_catid)) {
				if ($catids[0] != 0) {
					...
					foreach ($this->categorys as $catid => $cat) {
						$cat = getCategory($cat['catid']);
						$count_arrchildid = count(explode(',', $cat['arrchildid']));
						if ($count_arrchildid > 1 || $cat['type'] != 0)
							continue;
			...
			if (!isset($catid_arr[$autoid])) {
				$this->repair();
				$this->success("URL更新完成,开始更新首页", U("Createhtml/update_post","process=2"));
				exit;
			}
			$catid = $catid_arr[$autoid];
			...
			if ($pages > $page) {
				...
				$forward = U("Createhtml/update_post", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&autoid={$autoid}&set_catid=1&process=4");
				//$this->assign("waitSecond", 200);
				$this->success($message, $forward);
			} else {
				$autoid++;
			...
			if ($pages > $page) {
				...
				$forward = U("Createhtml/update_post", "type={$type}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&autoid={$autoid}&set_catid=1&process=5");
				$admintpl_path=C("SP_ADMIN_TMPL_PATH").C("SP_ADMIN_DEFAULT_THEME")."/";
				C("TMPL_ACTION_SUCCESS",$admintpl_path.C("SP_ADMIN_TMPL_ACTION_SUCCESS"));
				$this->success($message, $forward);
			} else {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if (!empty($this->categorys)) {
				foreach ($this->categorys as $catid => $r) {
					...
					if ($r['type'] == 2 && $count_arrchildid == 1)
						continue;
		...
		if(isset($_POST['dosubmit'])){
			...
			if ($modelid) {
				...
				if ($pages > $page) {
					...
					$forward = U("Content/Createhtml/update_urls", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&modelid={$modelid}");
					$this->assign("waitSecond", 200);
					$this->success($message, $forward);
				} else {
					//删除缓存
				...
				if ($pages > $page) {
					...
					$forward = U("Content/Createhtml/update_urls", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&autoid={$autoid}&set_catid=1");
					$this->assign("waitSecond", 200);
					$this->success($message, $forward);
				} else {
					$autoid++;
			...
			if (!empty($this->categorys)) {
				foreach ($this->categorys as $catid => $r) {
					...
					if ($modelid && $modelid != $r['modelid'])
						continue;
					$r['disabled'] = ($count_arrchildid > 1) ? 'disabled' : '';
					$categorys[$catid] = $r;
				}
			}
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($modelid) {
				...
				if ($pages > $page) {
					...
					$forward = U("Content/Createhtml/update_show", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&modelid={$modelid}");
					//$this->assign("waitSecond", 200);
					$admintpl_path=C("SP_ADMIN_TMPL_PATH").C("SP_ADMIN_DEFAULT_THEME")."/";
					C("TMPL_ACTION_SUCCESS",$admintpl_path.C("SP_ADMIN_TMPL_ACTION_SUCCESS"));
				...
				if ($pages > $page) {
					...
					$forward = U("Content/Createhtml/update_show", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&autoid={$autoid}&set_catid=1");
					$admintpl_path=C("SP_ADMIN_TMPL_PATH").C("SP_ADMIN_DEFAULT_THEME")."/";
					C("TMPL_ACTION_SUCCESS",$admintpl_path.C("SP_ADMIN_TMPL_ACTION_SUCCESS"));
					//$this->assign("waitSecond", 200);
					$this->success($message, $forward);
