...
class FieldController extends AdminbaseController {
    ...
    public function _initialize() {
        ...
        //字段类型存放目录APP_PATH . 'Content/Fields/'
    ...
    public function index() {
        ...
        $this->assign("data", $data);//print_r($this->modelfield->forbid_delete);exit;
        $this->display();
    }
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            $info=$this->modelfield->editField($post, $fieldid);
            //print_r($info);exit;
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            $info=$this->modelfield->addField($post);
            if ($info) {
                adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_ADD').L('ADMINLOG_ZIDUAN').$post['field']);
    ...
    public function delete() {
        ...
        $info = M('ModelField')->where(array('fieldid'=>$fieldid))->find();
        if (empty($fieldid)) {
            $this->error('字段ID不能为空！');
        }
        if ($this->modelfield->deleteField($fieldid)) {
            adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_DEL').L('ADMINLOG_ZIDUAN').$info['field']);
            $this->success("字段删除成功！");
        } else {
            $error = $this->modelfield->getError();
    ...
    public function listorder() {
        if (IS_POST) {
            ...
            S('Model', NULL);
            S('ModelField', NULL);
            adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_ORDER').L('ADMINLOG_ZIDUAN'));
            $this->success("排序更新成功！");
        } else {
            $this->error("排序失败！");
        }
    ...
    public function disabled() {
        ...
        if ($status) {
            adminlog($_SESSION['ADMIN_ID'],L('ADMINLOG_QIYONG').L('ADMINLOG_JINYONG').L('ADMINLOG_ZIDUAN').$field['field']);
            $this->success("操作成功！");
        } else {
            $this->error("操作失败！");
        }
    ...
    public function priview() {
        ...
        S('Model', NULL);
        S('ModelField', NULL);
        $disabled = getModel($modelid, 'disabled');
        if($disabled == 0){
            $content_form = new \Content\Fields\content_form($modelid);
            //生成对应字段的输入表单
