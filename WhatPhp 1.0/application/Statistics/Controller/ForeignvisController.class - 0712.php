...
class ForeignvisController extends AdminbaseController{
	...
	public function formateForeign_c($post){
		if($post){
			...
			$where['time'] = array(array('EGT',strtotime(date('Y-m-01', strtotime(date("Y-m-d"))))),array('ELT',strtotime("today")));
		}
		
		//点击量
		$count = M('Visits')->field("count(ip) as ip,sum(`clicks`) as `numclick`,FROM_UNIXTIME(time, '%Y-%m-%d' ) as time")->group("time")->where($where)->order('time desc')->select();
		...
		$list_c = M('Visits')->field("count(ip) as ip,sum(`clicks`) as `numclick`,FROM_UNIXTIME(time, '%Y-%m-%d' ) as time")->group("time")->where($where)->order('time desc')->limit($page->firstRow . ',' . $page->listRows)->select();
