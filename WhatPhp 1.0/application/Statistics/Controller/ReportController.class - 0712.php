...
class ReportController extends AdminbaseController{
	...
	public function formateForeign_c($post){
		if($post){
			...
			$where['time'] = array(array('EGT',strtotime("2010-01-01")),array('ELT',strtotime("today")));
		}
		
		//点击量
		$list_c = M('Visits')->field("count(ip) as ip,sum(`clicks`) as `numclick`,FROM_UNIXTIME(time, '%Y-%m-%d' ) as time")->group("time")->where($where)->order('time desc')->select();
		$nums=array();
...
//		$result[12]['parentid'] =0;
        ...
        $result[13]['id'] = 14;
        $result[13]['level'] =1;
        $result[13]['parentid_node'] ='class="child-of-node-9"';
        $result[13]['name'] = "访客地区分布";
        $result[13]['parentid'] =9;

        $result[14]['id'] = 15;
        $result[14]['level'] =1;
        $result[14]['parentid_node'] ='class="child-of-node-9"';
        $result[14]['name'] = "受访页面";
        $result[14]['parentid'] =9;

        $result[15]['id'] = 16;
        $result[15]['level'] = 1;
        $result[15]['parentid_node'] ='class="child-of-node-9"';
        $result[15]['name'] = "访问明细";
        $result[15]['parentid'] =9;
	...
	public function creport(){
		...
		foreach($typearr as $value){
            ...
            if ($value ==14) {  //访客地区分布
                $backfun = A('Browse')->formateCountry(array('start_time'=>$stime,'end_time'=>$etime));
                $broseCountryList = $backfun['allList'];
                $pieRel = A('Browse')->formateCountryBt(array('start_time'=>$stime,'end_time'=>$etime));
                $this->assign('pieCountrydata', $pieRel['rel_bt']);
                $this->assign('pieCountry', $pieRel['country_bt']);
                $this->assign('broseCountryList', $broseCountryList);
                continue;
            }
            if ($value ==15) {  //受访页面
                $backfun = A('Browse')->formatePage(array('start_time'=>$stime,'end_time'=>$etime));
                $brosePageList = $backfun['allList'];
                $pieRel = A('Browse')->formatePageBt(array('start_time'=>$stime,'end_time'=>$etime));
                $this->assign('piePagedata', $pieRel['rel_page_bt']);
                $this->assign('piePage', $pieRel['title_bt']);
                $this->assign('brosePageList', $brosePageList);
                continue;
            }
            if ($value ==16) {  //访问明细
                $backfun = A('Browse')->formateDetail(array('start_time'=>$stime,'end_time'=>$etime));
                $this->assign('broseDetailList', $backfun['allList']);
                continue;
            }
		}
	...
	*/
	public function zcpppdf(){
        ...
        //访客地区分布
        if ($data['brose_country']) {
            $data['brose_country'] = html_entity_decode($data['brose_country']);
        }
        //受访页面
        if ($data['brose_page']) {
            $data['brose_page'] = html_entity_decode($data['brose_page']);
        }
        //访问明细
        if ($data['brose_detail']) {
            $data['brose_detail'] = html_entity_decode($data['brose_detail']);
        }
