...
class BrowseController extends AdminbaseController
{
    ...
    public function visitorNumber($country, $post = '')
    {
        ...
        //新访客
        $listIp = M('Visits')
            ->field("country,ip,count(ip) as num")
            ->where($where)
            ->group('ip')
            ->order("time asc")
            ->select();
        ...
        $data['visit_new_num'] = count($newIps);
    ...
    public function formatePage($post)
    {
        ...
        $list_old = M('Visits')
            ->field("title, url, sum(`clicks`) as `num`")
            ->where($where)
            ->group('url')
            ->order('num desc')
            ->select();
        foreach ($list_old as $key => $value) {
            $visitInfo = $this->visitorPageNumber($value['url'], $post);
            $list_old[$key]['visit_num'] = $visitInfo['visit_num'];
        ...
        foreach ($list as $key => $value) {
            $visitInfo = $this->visitorPageNumber($value['url'], $post);
            $list[$key]['visit_num'] = $visitInfo['visit_num'];
            $list[$key]['visit_new_num'] = $visitInfo['visit_new_num'];
            $list[$key]['new_rate'] = $visitInfo['new_rate'];
