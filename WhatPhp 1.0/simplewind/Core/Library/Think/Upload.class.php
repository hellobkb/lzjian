...
class Upload {
    ...
    public function uploadOne2($file){
        $info = $this->upload_s1(array($file));
        return $info ? $info[0] : $info;
    }
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            $savename = $this->imgname?$this->imgname:$this->getSaveName($file);
            if(false == $savename){
                continue;
            } else {
    ...
    public function upload2($files='') {
     ...
     public function upload_s1($files='') {
        if('' === $files){
            $files  =   $_FILES;
        }
        if(empty($files)){
