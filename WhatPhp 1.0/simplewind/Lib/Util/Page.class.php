...
class Page {
    ...
    function __construct($Total_Size = 1, $Page_Size = 20, $Current_Page = 1, $List_Page = 6, $PageParam = 'p', $PageLink = '', $Static = FALSE) {
        ...
        $this->Page_tpl ['default'] = array('Tpl' => '<div class="pager"><span style="margin-right: 10px;">共<b style="color:#2baab1;">{zcount}</b>条数据</span>{first}{prev}{liststart}{list}{listend}{next}{last} 跳转到 {jump} 页</div>', 'Config' => array());
    ...
    public function GetCurrentPage() {
        ...
        if(isset($_GET[C('VAR_PAGE')]) && $_GET[C('VAR_PAGE')]>1){
            $this->Current_page=$_GET[C('VAR_PAGE')];
        }
        $this->Current_page = ($this->Current_page <=$total_pages? $this->Current_page : $total_pages);
    }
    ...
    public function Pager($Page_tpl = '') {
        ...
        for ($i = $pStart; $i <= $pEnd; $i++) {
            if ($i == $cfg ['pageindex']) {
                ...
                $pList .=$wraper.'<a class="' . $cfg ['currentclass'] . '" >' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
            } else {
        ...
        //上一页 首页
        if($cfg ['pageindex'] == 1 && $Page_tpl['Config']['ss']){
            if ($this->Static) {
                $pFirst = $this->linkwraper_pre.'<a class="big">' . $cfg ['first'] . '</a> '.$this->linkwraper_after; //首页
            } else {
                $pFirst = $this->linkwraper_pre.'<a class="big">' . $cfg ['first'] . '</a> '.$this->linkwraper_after; //首页
            }
            if ($this->Static && ($cfg ['pageindex'] - 1) == 1) {
                $pPrev = $this->linkwraper_pre.'<a class="big">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after; //上一页
            } else {
                $pPrev = $this->linkwraper_pre.'<a class="big no">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after;
            }
        }elseif ($cfg ['pageindex'] > 1) {
            if ($this->Static) {
            ...
            if ($this->Static && ($cfg ['pageindex'] - 1) == 1) {
                $pPrev = $this->linkwraper_pre.'<a href="' . $this->PageLink['index'] . '" class="big">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after; //上一页
            } else {
                $pPrev = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pageindex'] - 1, $cfg ['link']) . '" class="big">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after;
            }
        }
        //下一页，尾页
        if($cfg ['pageindex']==$cfg ['pagecount'] && $Page_tpl['Config']['ss']){
            $pLast = $this->linkwraper_pre.'<a class="big">' . $cfg ['last'] . '</a> '.$this->linkwraper_after;
            $pNext = $this->linkwraper_pre.'<a class="big no">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
        }elseif($cfg ['pageindex'] < $cfg ['pagecount']) {
            $pLast = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pagecount'], $cfg ['link']) . '">' . $cfg ['last'] . '</a> '.$this->linkwraper_after;
            $pNext = $this->linkwraper_pre.'<a class="big" href="' . str_replace('*', $cfg ['pageindex'] + 1, $cfg ['link']) . '">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
        }
        ...
        $patterns = array('/{zcount}/', '/{recordcount}/', '/{pagecount}/', '/{pageindex}/', '/{pagesize}/', '/{list}/', '/{liststart}/', '/{listend}/', '/{first}/', '/{prev}/', '/{next}/', '/{last}/', '/{jump}/');
        $replace = array($this->Total_Pages, $cfg ['recordcount'], $cfg ['pagecount'], $cfg ['pageindex'], $cfg ['pagesize'], $pList, $pListStart, $pListEnd, $pFirst, $pPrev, $pNext, $pLast, $pJump);
