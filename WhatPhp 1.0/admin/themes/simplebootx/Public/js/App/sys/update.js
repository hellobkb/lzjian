...
(function(w) {
	...
	var server = "http://192.168.0.6:8020/DownloadLink/update.json", //获取升级描述文件服务器地址
		localDir = "update",
						...
						dir.getFile(localFile, {
							...
							fentry.createWriter(function(writer) {
								writer.onerror = function() {
									plus.nativeUI.toast("获取升级数据，保存文件失败");
								...
								plus.nativeUI.toast("获取升级数据，创建写文件对象失败");
								console.log("获取升级数据，创建写文件对象失败：" + e.message);
							});
						}, function(e) {
							plus.nativeUI.toast("获取升级数据，打开保存文件失败");
							console.log("获取升级数据，打开保存文件失败：" + e.message);
						});
					} else {
						plus.nativeUI.toast("获取升级数据，联网请求失败");
